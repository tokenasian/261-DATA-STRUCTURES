...
class LinkedList:
    ...
    def insert_at_index(self, index: int, value: object) -> None:
        ...
        curr = self._head  # initialize pointer

        while count != index:  # traverse through the list, locate the specified index position 
            curr = curr.next
            count += 1  # increment the count by 1

        node = SLNode(value)  # insert the new node
        node.next = curr.next  # update the pointers
        curr.next = node
    ...
    def remove_at_index(self, index: int) -> None:
        ...
        temp = current.next.next  # update the pointers
        current.next = temp  
